...
var svgPath = __dirname + '/chart.svg';
var pngPath = __dirname + '/chart.png';

fs.writeFile(svgPath, svg, function () {
  console.log('Wrote ' + svg.length + ' bytes to ' + svgPath); // eslint-disable-line no-path-concat
  pnfs.readFile(svgPath)
    .then(svg2png)
    .then(  function(buffer){pnfs.writeFile(pngPath, buffer);})
